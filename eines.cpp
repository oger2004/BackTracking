...
vector < string > tokens ( const string & s , char separador , bool cometes ) {
 ...
 string token ( const string & s , char separador , bool cometes , long & primer , long & ultim ) {
     string t;
    if(!cometes || s[primer]!='"') { // No volem tenir en compte les " o no comen¸c a per "
        ...
        if (ultim == string :: npos){
    ...
    else { // comen¸c a per " i les volem tenir en compte com a delimitadors
         primer ++;
         ultim = s. find ('"', primer );
         ...
         if ( ultim == string :: npos ) {
            throw (" cometes no tancades " );
             ...
             if ( primer > s. length ()) {
                ultim = string :: npos ; // era l'´u ltim token
             }
         }
     }
     return t;
